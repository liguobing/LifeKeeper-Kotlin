...
class AddBillCategoryListAdapter(
    private var context: Context,
    private var list: ArrayList<BillCategory>
) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        val view = LayoutInflater.from(context).inflate(R.layout.view___add_bill___category_list_item,parent,false)
        if(position%2 == 0){
            view.setBackgroundColor(Color.parseColor("#E4E2E2"))
        }
        val accountName = view.findViewById<TextView>(R.id.tv_account_name)
        accountName.text = list[position].categoryName
        return view
    }
}